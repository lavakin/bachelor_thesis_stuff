...
class Mol ():
    ...
    def vertices(self):
        ...
        return list(range(self.__Order))
    ...
    def read_edgelist(self, edges):
        ...
        d = dict(list(zip(nodes,list(range(len(nodes))))))
    ...
    def incidence_matrix(self):
        ...
        if self.__B == []:
            ...
            b = [func(e) for e in self.edges()]
            # transpose the result
            self.__B = list(map(list, list(zip(*b)))) 
        return self.__B
    ...
    def degrees(self):
        ...
        """
        if self.__Degrees == []:
            self.__Degrees = [sum(r) for r in self.__A]
    ...
    def distances_from_vertex(self, v):
        ...
        while nextlevel:
            ...
            for v in thislevel:
                if v not in seen: 
                    seen[v]=level 
                    nb = [i for (i,j) in zip(list(range(len(self.__A[v]))), self.__A[v]) if j!=0]
                    nextlevel.extend(nb)
        ...
        if type(matrix) is str:
            ...
            if matrix == "adjacency" or matrix == "A":
                ...
                if self.__Laplacian_spectrum == []:
                    ...
                    s.sort(reverse=True)
                    self.__Laplacian_spectrum = [x if x>0 else 0 for x in s]
                ...
                if self.__Signless_laplacian_spectrum == []:
                    ...
                    s.sort(reverse=True)
                    self.__Signless_laplacian_spectrum = [x if x>0 else 0 for x in s]
    ...
    def energy(self, matrix="adjacency"):
        ...
        return np.float64(np.sum( [abs(x-a) for x in s], dtype=np.longdouble))
    ...
    def zagreb_m2_index(self):
        ...
        return sum( [self.degrees()[e1_e27[0]]*self.degrees()[e1_e27[1]] for e1_e27 in self.edges()] )    
    ...
    def connectivity_index(self, power):
        ...
        if len(E) == 0: return 0
        return np.float64(np.sum( [( self.degrees()[e1_e2[0]]*self.degrees()[e1_e2[1]] ) ** power for e1_e2 in E] , dtype=np.longdouble))
    ...
    def augmented_zagreb_index(self):
        ...
        d = self.degrees()
        if len(E) < 2: return 0
        return np.float64(np.sum( [(np.longdouble(d[e1_e22[0]]*d[e1_e22[1]]) / (d[e1_e22[0]]+d[e1_e22[1]]-2)) **3 for e1_e22 in E] , dtype=np.longdouble))

    def sum_connectivity_index(self):
        """ Sum-Connectivity index"""
        E = self.edges() # E - all edges
        if len(E) == 0: return 0
        return np.float64(np.sum( [( self.degrees()[e1_e23[0]]+self.degrees()[e1_e23[1]] ) ** (-0.5) for e1_e23 in E] , dtype=np.longdouble)) 
        
    def geometric_arithmetic_index(self):
        """ Geometric-Arithmetic index"""
        E = self.edges() # E - all edges
        if len(E) == 0: return 0
        return np.float64(np.sum( [2.0*np.sqrt(self.degrees()[e1_e24[0]]*self.degrees()[e1_e24[1]] ) / (self.degrees()[e1_e24[0]]+self.degrees()[e1_e24[1]]) for e1_e24 in E] , dtype=np.longdouble)) 
    ...
    def estrada_index(self, matrix = "adjacency"):
        ...
        """
        return np.float64(np.sum( [np.exp( x.real ) for x in self.spectrum(matrix)] ,dtype=np.longdouble )) 
    ...
    def balaban_j_index(self):
        ...
        return np.float64(k * np.sum( [1 / np.sqrt((ds[u_v[0]][0,0]*ds[u_v[1]][0,0])) for u_v in self.edges()], dtype=np.longdouble))
    ...
    def sum_balaban_index(self):
        ...
        m = len(self.edges())
        k = (m / ( m - self.__Order +2.0 ))
        return np.float64(k * np.sum( [1 / np.sqrt((ds[u_v1[0]][0,0]+ds[u_v1[1]][0,0])) for u_v1 in self.edges()], dtype=np.longdouble))
    ...
    def LEL(self):
        ...
        return np.float64(np.sum( [np.sqrt(x) for x in self.spectrum('laplacian')], dtype=np.longdouble))

    def multiplicative_sum_zagreb_index(self):
        """ Log( Multiplicative Sum Zagreb index )""" 
        d = self.degrees()
        return np.float64(np.sum( [np.log(np.float64(d[u_v5[0]]+d[u_v5[1]])) for u_v5 in self.edges()] , dtype=np.longdouble))
        
    def multiplicative_p2_zagreb_index(self):
        """Calculates Log( Multiplicative P2 Zagreb index )""" 
        d = self.degrees()
        return np.float64(np.sum( [np.log(np.float64(d[u_v6[0]]*d[u_v6[1]])) for u_v6 in self.edges()] , dtype=np.longdouble))
    
    def multiplicative_p1_zagreb_index(self):
        """Calculates Log( Multiplicative P1 Zagreb index )""" 
        d = self.degrees()
        return np.float64(np.sum( [np.log(np.float64(d[v]**2)) for v in self.vertices()] , dtype=np.longdouble))
    ...
    def adriatic_index(self,p,i,j,a):
        ...
        func  = lambda u_v8 : self._adriatic_entry_(np.float64(d[u_v8[0]]), np.float64(d[u_v8[1]]), i,j,a)
        return np.float64(np.sum( list(map( func, self.edges())) , dtype=np.longdouble))
